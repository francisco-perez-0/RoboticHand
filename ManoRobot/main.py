...
if usar == "si":
    ...
    try:
        ...
        print(f"Error: {e}")
        conexionSerie = False
        input("Presiona Enter para continuar sin conexión en serie...")
# ---------------------- FIN ENVIO EN SERIE ----------------------
...
datosAEnviar = [0]*5  # Inicializo arreglo con caracteres vacíos
...
while True:
    ...
    if manos:
        ...
        for i in range(5):
            ...
            datosAEnviar[i] = round(numpy.interp(distancias[i], [175, 425], [0, (pasos - 1)]))
                ...
                if recepcion == 'b':
                    ...
                    for i in range(5):
                       # print(dedos[i], "enviado!")
                        ser.write(datosAEnviar[i].to_bytes(1, byteorder='big'))  # Envía el dato
    ...
    if tecla == 78 or tecla == 110: # Si se presiona la tecla N/n
        print("CAMARA EN PAUSA\n")
        ingreso = input("Ingrese nuevo nivel de muestreo\n[1, 2, 3, 4, 5, 6, 7, 8]:  ")
        if conexionSerie:
            ser.close()
            continuar = input("Presione el botón ubicado en la PCB y presione ENTER para reconectar a la EDU CIAA NXP")
